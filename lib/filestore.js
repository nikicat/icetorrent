...
const Store = Class({
    ...
    initialize: function(info) {
        ...
        this.debug('initializing. piece length '+this.pieceLength+'; piece count '+this.pieceCount);
    },

    parseMultipleFiles: function(info, destDir) {
        var files = [];
        var totalLength = 0;
        for each (file in info.files) {
            files.push({
        ...
        this.debug('files: ' + JSON.stringify(this.files));
    },
    ...
    get webInfo() {
        return {
            ...
            'neede': this.neede.length
        };
    },

    filterPieces: function(value) {
        return this.goodPieces.getBitArray().filter(function(val){ return val == value; }).length
    },
    ...
    inspect: function(callback){
        ...
        function inspectCallback(pieceIndex, err){
            ...
            if (!goodPiece) {
                this.left += this.getPieceLength(pieceIndex);
    ...
    onPieceFragment: function(index, offset, data) {
        ...
        var pieceLength = (index === this.pieceCount - 1) ? this.lastPieceLength : this.pieceLength;
            ...
            if (hasdone === Math.ceil(pieceLength/fragmentSize)){
                ...
                this.inspectPiece(index, (function(error) {
                    if (!error) {
                        ...
                        emit(this, 'have:piece', index);
                        this.notifyAboutPieces();
                    } else {
                        this.warning('waah broken piece: ', error);
                    }
                }).bind(this))
            } else {
    ...
    notifyAboutPiecesImpl: function(lastAvailable) {
        if (this.notifiedPieces < lastAvailable) {
            this.readPiece(this.notifiedPieces, (function (error, data) {
                if (error) {
                    this.error('failed to read piece '+this.notifiedPieces, error);
                } else {
                    if (data) {
                        emit(this, 'data', data);
                    } else {
                        this.notifiedPieces++;
                        this.notifyAboutPiecesImpl(lastAvailable);
                    }
                }
            }).bind(this));
        } else if (this.notifiedPieces === this.pieceCount) {
            emit(this, 'end');
        }
    },

    notifyAboutPieces: function() {
        let availableFromStart = this._availableFromStart();
        this.debug('available from start '+availableFromStart+' pieces. notified about '+this.notifiedPieces+' pieces');
        this.notifyAboutPiecesImpl(availableFromStart);
    },

    _availableFromStart: function() {
        let available = this.goodPieces.getBitArray().indexOf(false);
        return available == -1 ? this.pieceCount : available;
    },
