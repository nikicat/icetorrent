...
const HttpTracker = Logged.compose({
    tag: null,
    constructor: function(url) {
        this.url = url;
        this.tag = 'HttpTracker<'+url.host+'>';
    },
    ping: function(params, callback) {
        var existingParams = querystring.parse(this.url.query);
        for (name in existingParams) {
            params[name] = existingParams[name];
        }
        this.debug('base url: '+this.url.spec);
        var trackerUrl = url.URL(this.url.filePath + '?' + querystring.stringify(params), this.url);
        this.log('pinging tracker '+trackerUrl.spec);
            ...
            if (statusCode == 200) {
                ...
                if (statusCode >= 300 && statusCode <= 399) {
                    if (redirectLimit <= 0) {
                        ...
                        this.log('redirect ' + statusCode + ' ' + JSON.stringify(body));
