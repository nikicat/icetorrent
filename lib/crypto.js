...
function createHash(algorythm) {
    var hash = new sjcl.hash[algorythm]();
    hash.digest = function (mode) {
        if (mode == 'binary') {
            return hash.finalize().join();
        } else {
            throw 'unimplemented digest mode: ' + mode;
        }
    };
    return hash;
}
...
exports.createHash = function (algorythm) {
    return new Hash(algorythm);
};