...
let layout = new log4javascript.PatternLayout('[%d] %c[%f]: %m');
...
function getLevel(name) {
    ...
    logLogger.debug('pref value for log.'+name+' is '+levelstr);
...
function getLogger(name, tag) {
    ...
    logLogger.debug('setting level for '+name+' to '+level);
...
let LoggedProto = {
    _log: function(method, args) {
        ...
        this._logger[method].apply(this._logger, [].concat([args[i] for (i in args)], [this]));
    },
    toString: function() {
...
for (let fun of ['log', 'trace', 'debug', 'info', 'warning', 'error', 'fatal', 'assert']) {
    let f = fun;
