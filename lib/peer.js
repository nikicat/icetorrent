...
const Peer = Class({
    ...
    initialize: function(conn){
        this.address = conn.host+':'+conn.port;
    ...
    handleHave: function(data){
        let index = readInt(data);
        this.debug('have ' + index);
        this.goodPieces.set(index, 1);
    ...
    processMessage: function(){
        ...
        if (this.input.length < 4) {
            this.debug('waiting for length');
            return false;
        }
        var dataLen = readInt(this.input);
        if (this.input.length < dataLen + 4) {
            this.debug('waiting for data (need '+(dataLen + 4 - this.input.length)+' more bytes)');
            return false;
        }
        if (dataLen == 0) {
            // Keep alive;
    ...
    have: function(piece){
        this.debug('sending have '+piece.index);
        this.writePacket(4, encodeInt(piece.index));
    },
