...
const Peer = Class({
    ...
    processMessage: function(){
        ...
        if (this.needHeader) {
            ...
            if (this.checkHeader(input)) {
                ...
                if (this.peerFlags.charCodeAt(5) & 0x10) {
                    this.trace('peer supports extension protocol');
        ...
        if (dataLen == 0) {
            ...
            if (id == 0) {
                ...
                this.trace('received message with unknown id '+id+' and length '+dataLen);
    ...
    set choked (state){
        if (state != this.amChoked) {
            this.amChoked = state;
            this.trace('sending choked flag '+state);
    ...
    set interested (state){
        if (state != this.amInterested) {
            ...
            this.trace('sending interested flag '+state);
    ...
    sendPiece: function(index, begin, data){
        this.trace('sending piece '+index+' from offset '+begin+' with length '+data.length);
        this.writePacket(7, encodeInt(index) + encodeInt(begin) + data);
    },
    sendCancel: function(index, begin, length){
        this.trace('sending cancel for piece '+index+' from offset '+begin+' with length '+length);
    ...
    sendExtendedHandshake: function() {
        ...
        this.trace('sending extension protocol handshake m='+JSON.stringify(m));
