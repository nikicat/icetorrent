...
const {MagnetLoader, parse} = require('./magnet');
...
let Channel = Class({
    ...
    initialize: function(uri) {
        ...
        this.tag = base64.encode(parsed.infoHash.substring(0, 5));
        let torrent = TorrentManager.torrents[parsed.infoHash];
        if (torrent !== undefined) {
            this.debug('torrent already loaded');
            channel.contentStream = fs.open(torrent.path, 'rb').input;
        } else {
            ...
            this.asyncOpen = this.asyncOpenMagnet;
        }
    },
    ...
    asyncOpenMagnet: function(listener, context) {
        try {
