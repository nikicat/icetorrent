...
function decodeList(s){
    ...
    while (true) {
        if (s.length === 0) {
            throw new Error("end of input while looking for 'e'");
        }
        if (s.charAt(0) === 'e') {
...
exports.decode = function decode(s, options){
    try {
        var result = decode2(s);
        var leftOver = result[1];
        if (leftOver !== '' && (options === undefined || options.withLeftover === false)) {
            throw new Error("characters left over at end of decode: '" + leftOver + "'");
        }
    } catch (e) {
        console.error('failed to decode '+s+': '+e);
        throw e;
    }
    return (options !== undefined && options.withLeftover) ? result : result[0];
};
