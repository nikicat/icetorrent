    ...
    return {
            ...
            tracker.ping(trackerClient, params, function(error, response){
                ...
                if (!error) {
                    ...
                    if ('peers6' in response) {
                        ...
                        for (var i = 0; i < numPeers; i++) {
                            that.addPeer(newPeers.substring(i * 18, (i + 1) * 18));
                        }
                    }
                }
                else {
                ...
                that.pingTimer = timer.setTimeout(that.pingTracker.bind(that, trackerClient), interval * 1000);
            });
        },
        ...
        start: function(){
            ...
            fs.readFile(this.torrentPath, 'binary', function startTorrentCallback(error, contents){
                if (error) {
                    console.error('could not open torrent file ' + that.torrentPath + ': ' + error);
                    console.exception(error);
                }
                else {
                    ...
                    if ('comment' in that.metaInfo) {
                        console.log('Torrent \'' + that.metaInfo.comment + '\'');
                    }
                    ...
                    filestore.inspect(that.store, function inspectCallback(error){
                        if (error) {
                            ...
                            that.trackerClients = tracker.create(that.metaInfo);
                            for (i in that.trackerClients) {
                                that.pingTracker(that.trackerClients[i]);
                            }
...
*/
                                ...
                                console.log('# of peers: ' + Object.keys(that.peers).length);
