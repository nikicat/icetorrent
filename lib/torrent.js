...
var Torrent = Logged.compose({
    tag: 'Torrent<>',
    constructor: function(torrentPath, destDir) {
    ...
    start: function(){
        ...
        fs.readFile(this.torrentPath, 'binary', (function startTorrentCallback(error, contents){
            if (error) {
                this.error('could not open torrent file ' + this.torrentPath + ': ' + error);
                console.exception(error);
            }
            else {
                ...
                if ('comment' in that.metaInfo) {
                    this.log('comment: "' + this.metaInfo.comment + '"');
                }
...
*/
                            this.debug('pieces sorted by availability (rarest first). ('+pieces_array.length+') :'+pieces_array.join(', '));
