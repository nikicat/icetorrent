...
function choice(arr, count=1) {
    arr = [arr[key] for (key of Object.keys(arr))];
    let results = [];
    for (var i=0; i<count; ++i) {
        results.push(arr[Math.floor(arr.length * Math.random())]);
    }
...
const Torrent = Class({
    ...
    addPeer: function(peerAddress){
        if (!(peerAddress in this.peers)) {
            try {
                this.peers[peerAddress] = Peer(peerAddress, decodeHost(peerAddress), decodePort(peerAddress), this);
            }
            catch (e) {
    ...
    requestMetadata: function() {
        for (var i=0; i<this.metadataPieces.length; ++i) {
            if (this.metadataPieces[i] === undefined) {
                ...
                let randomPeers = choice(this.peers, 2);
    ...
    addMetadataPiece: function(piece, data) {
        ...
        if (pieceData !== undefined && pieceData !== data) {
            ...
            if (this.metadataPieces.every(function (data) data !== undefined)) {
                ...
                this.store = Store(this.info, this.destDir);
    ...
    start: function(dataCallback=function() {}, progressive=false) {
        ...
        this.mainLoop = timers.setInterval((function() {
            var pieces = this.getPiecesToRequest(sortByAvailability=progressive===false);
    ...
    requestPieces: function(pieces) {
        ...
        console.info(gotParts + "/" + totalParts + " recieved (" + (Math.floor((gotParts / totalParts) * 100 * 100) / 100) + "%)");
    ...
    replyToRequests: function() {
        ...
        this.info('# of peers: ' + Object.keys(this.peers).length);
    }, 
