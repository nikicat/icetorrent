...
const Swarm = Class({
    ...
    onHave: function(index) {
        this.available[index] = (this.available[index] || 0) + 1; //add it to a map of pieces (since zero = dont have, 1 = have, adding works)
        this.debug('availability for piece '+index+' is '+this.available[index]);
        emit(this, 'have', index);
    },

    getBitfield: function(len) {
        let result = [];
        for (let i=0; i<(len+1)/8; i++) {
            result.push(0);
        }
        for each (let piece in this.available) {
            result[piece.index/8] |= piece.index % 8;
        }
        return result;
    },
        ...
        if (!this.peerAddresses.has(peerAddress)) {
            try {
                ...
                conn.on('error', (function(e) {
                    this.info('could not connect to peer '+decodeAddress(peerAddress));
                    this.debug('connection to peer '+decodeAddress(peerAddress)+' aborted: '+e);
    ...
    requestPieceFragment: function(piece, offset, size) {
        ...
        for each (let peer in choice(this.peers, Object.keys(this.peers).length)) {
            ...
            if (peer.getBitfield().getBitArray()[piece] /*&& !peers_random[i].peerChoked*/) {
...
exports.Swarm = function (options) {
    let swarm = swarms[options.infoHash];
    if (swarm === undefined) {
        swarms[options.infoHash] = swarm = Swarm(options);
    }
    return swarm;
}
